...
export default function BrokerForm({ broker, onSuccess, onCancel }: Props) {
  ...
  const [cnpj, setCnpj] = useState("");      // state interno sempre string
  ...
  useEffect(() => {
    if (broker) {
      setName(broker.name);
      setCnpj(broker.cnpj ?? "");          // ← garante string, mesmo se vier null
      setWebsite(broker.website ?? "");    // ← idem
    } else {
      clearFields();
    }
  }, [broker, clearFields]);
    ...
    try {
      if (broker) {
        await updateBroker({
          id: broker.id,
          name,
          cnpj: cnpjDigits,                 // ← envia sem máscara pro backend
