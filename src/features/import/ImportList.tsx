...
import { getImportJobs } from "./import.service";
...
export default function ImportList() {
  ...
  const [errorsList, setErrorsList] = useState<ImportErrorRow[] | null>(null);
  ...
  async function handleProcessValid(jobId: string): Promise<void> {
    try {
      ...
      // TODO: chamar serviço para processar todas as linhas válidas do job:
      // await processImportJobValidRows(jobId);
      // TODO: após sucesso, recarregar a lista:
      // await reload();
    } catch (e) {
      // TODO: opcional - usar NotificationService/toastr
  ...
  async function openErrors(job: ImportJobDTO): Promise<void> {
    ...
    try {
      // TODO: chamar serviço para buscar erros deste job:
      // const data = await getImportErrors(job.id);
      // setErrorsList(data);
      // por enquanto, mantém em branco (ou exemplo vazio)
      setErrorsList([]);
    } catch (e) {
...
// ---------- Modal de erros (acessível e tipado) ----------
function ErrorsModal(props: {
  ...
  const { open, onClose, job, loading, loadError, errors } = props;
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-3xl bg-slate-900 rounded-2xl shadow-lg p-4 sm:p-6">
        ...
        {!loading && !loadError && errors && errors.length === 0 && (
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="text-left text-slate-300 bg-slate-800/50">
                  ...
                  <th className="py-2 px-3">Conteúdo bruto</th>
                </tr>
              </thead>
              <tbody>
                {errors.map((er) => (
                  <tr key={er.id} className="border-t border-slate-800 text-slate-200">
                    <td className="py-2 px-3 font-mono">{er.rowNumber}</td>
